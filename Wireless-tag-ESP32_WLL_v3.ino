...
////////////////////////////////////////////////////////////////////////////////////
String version_num = "David's WLL v2";
#include <WiFi.h>
#include <TFT_eSPI.h>
#include <SPI.h>
#include <time.h>
...
const int   daylightOffset_sec = 3600;  // Set your offset in seconds for Daylight saving
...
void loop() {
  ...
  if (millis() > previousMillis + loopDelay || Refresh) {
    ...
    display_text(10, 55, String(Temperature, 1) + char(247), GREEN, 3); // char(247) is ° symbol
    String PressureTrend_Str = "~"; // Either steady, climbing or falling
    if (PressureTrend > 0) PressureTrend_Str = "+";
    if (PressureTrend < 0) PressureTrend_Str = "-";
    display_text(125, 60, String(Humidity, 0) + "% " + PressureTrend_Str + String(AirPressure, 0) + "hPa", GREEN, 2);
    display_text(10, 100, "Dew Point  = " + String(Dewpoint, 1)    + char(247), CYAN, 2);
    display_text(10, 120, "Wind Chill = " + String(windchill, 1)   + char(247), CYAN, 2);
    display_text(10, 140, "Solar rad. = " + String(solarradiation) + "W/m2", CYAN, 2);
    display_text(10, 160, "Solar kW   = " + String(solarradiation * 1500 / 135 /  1000.0, 2) + "kW", CYAN, 2);
    display_text(10, 180, "Rainfall   = " + String(RainFall, 1)    + "mm", CYAN, 2);
    display_text(10, 200, "Rain-Rate  = " + String(RainRate, 1)    + "mm/hr", CYAN, 2);
    if (WiFi.status() == WL_CONNECTED) display_text(5, 15, "Wi-Fi", RED, 1);
    else StartWiFi();
    DisplayWindDirection(375, 120, winddirection, windspeed, 75, YELLOW, 2);
    ...
    display_text(10, 265, forecast, CYAN, 2);
  }
}
...
void StartWiFi() {
  ...
  WiFi.disconnect();       // Diconnect any previous connections
